...
for iSub = subList
    ...
    for iTrial = 1:length(dataFiles)
        ...
        if contains(subjectFile,'warmup')
        ...
        subjectNumber = subjectFile(k+3); % need to find the subject file number since that isn't actually how this program works and subject number doesn't matter
        ...
        for iS = 1:expData(iSub,iTrial).segments.nSeg
            ...
            expData(iSub,iTrial).segments.meanError{iS} = mean(headingError);
...
nConditions = 7; % includeding control
% 1: control
% 2: single, 8
% 3: single, 12
% 4: single, 16
...
% 7: gauss, 16
nSub = length(subList);
errorMat = zeros(nConditions,nSub);
timeMat =  zeros(nConditions,nSub);
lengthMat =  zeros(nConditions,nSub);
...
for iSub = subList
    for iTrial = 1:length(expData(iSub,:))
        ...
        if strcmp(expData(iSub,iTrial).condition,'control')
            ...
            if strcmp(expData(iSub,iTrial).condition,'single')
                switch expData(iSub,iTrial).belt
                    case 8
                        iCond = 2;
                    case 12
                        iCond = 3;
            ...
            if strcmp(expData(iSub,iTrial).condition,'gaussian')
                switch expData(iSub,iTrial).belt
                    case 8
                        iCond = 5;
                    case 12
        ...
        foob = [foob iCond];
        errorMat(iCond,iSub) =  errorMat(iCond,iSub) + mean(cell2mat(expData(iSub,iTrial).segments.meanError));
        timeMat(iCond,iSub) = timeMat(iCond,iSub) + expData(iSub,iTrial).pathTime;
        lengthMat(iCond,iSub) = lengthMat(iCond,iSub) + expData(iSub,iTrial).pathLength;
...
errorMat(1,:) = errorMat(1,:) / 6; % control condition has 6 per subject
timeMat(1,:) = timeMat(1,:) / 6;
lengthMat(1,:) = lengthMat(1,:) / 6;

errorMat(2:end,:) = errorMat(2:end,:) / 5
timeMat(2:end,:) = timeMat(2:end,:) / 5
lengthMat(2:end,:) = lengthMat(2:end,:) / 5
