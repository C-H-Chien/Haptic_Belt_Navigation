class constants:
    ...
    MSG_START = 1  # Represents the start of a data stream - needs to match arduino code, must be >0, <26
    MSG_END = 2  # Represents the end of a data stream - needs to match arduino code, must be >0, <26
    ...
    REPS_PER_COND = 1  # Number of repetitions for each of the 4 variable combinations
    ...
    REORIENT_PERIOD = 3  # Time subject has to stay at poleToGoTo during reorientation before new trial starts
    REORIENT_RADIUS = 0.5  # Margin of error allowed for reorientation at poleToGoTo

    REORIENT_YAW_ERROR = 10  # Margin of error allowed in yaw for reorientation while facing poleToFace
    REORIENT_PITCH_ERROR = 10  # Margin of error allowed in pitch for reorientation while facing poleToFace
