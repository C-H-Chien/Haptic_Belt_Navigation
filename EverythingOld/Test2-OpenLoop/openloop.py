...
#	emergencyWallsNormal.popWalls(curPos)
	...
	if distToDest < 0.75 or (finished == 2 and curPos[0] > 6) or (finished == 3 and
	 curPos[0] < 2) or (finished == 3 and curPos[2] > 9) or (finished == 4 and curPos[0] < -4) or (finished == 5 and
	  curPos[2] < 3) or (finished == 6 and curPos[2] < -6) or (finished == 6 and curPos[0] < -7) or (finished == 7 and
	   curPos[0] > 4) or (finished == 8 and curPos[0] < 0):
		print(finished)
		finished += 1
		if finished == 9:
			index = numMotors
			...
			tm.sleep(5.0)
			numpy.save('openloop_positions.npy', recordPos)
			numpy.save('openloop_orientations.npy', recordOrient)
			ser.write(struct.pack('>B', 255))
			sys.exit(0)
		else:
			directionsToSend = [255]
			for  i in range(len(motors[numMotors][index])):
				directionsToSend.append(motors[numMotors][index][i])
			directionsToSend.append(250)
			for num in directionsToSend:
				ser.write(struct.pack('>B', num))
			tm.sleep(3.0)
