...
polePosition1 = [0, 0, 6.5]
polePosition2 = [5, 0, 5] # x,y,z where y is vertical axis
...
if motionModel=='go_to_pole':
	...
	box2 = vizshape.addBox(size=(3, 5, 7)) ; box2.color(viz.GRAY)
...
def masterLoop(num):
	global time, polePosition1, polePosition2, directionsToSend, motionModel, ser, record, start, pystart, recordPos, recordOrient, finished
...
#	emergencyWallsNormal.popWalls(curPos)
	...
	for weight in motorDotProducts:
		weights.append(int(numpy.interp(weight, [0.2, 0.36], [3, 5])))
	if finished == 0:
	...
	if distToDest < 0.5 and finished == 0:
		index = numMotors ;
		directionsToSend = motors[numMotors][numMotors] ; directionsToSend.insert(0, 255)
		directionsToSend.append(254) ; directionsToSend.append(200)
		...
		numpy.save('positions_1A.npy', recordPos)
		numpy.save('orientations_1A.npy', recordOrient)


	if tm.time() - start > 0.1:
		count = 0
		while count < 5:
			directionsToSend = [255] ; strToWrite1 = "" ; strToWrite2 = ""
			for i in range(len(indices)):
				if weights[i] > count:
					directionsToSend.append(motors[numMotors][indices[i]][0])
					strToWrite1 += str(directionsToSend[-1]) ; strToWrite1 += '|'
					strToWrite2 += str(weights[i]) ; strToWrite2 += '|'
			if len(directionsToSend) > 1:
				file = open('motors.txt', 'w')
				file.write(strToWrite1 + '\n') ;
				file.write(strToWrite2) ; file.close()
				directionsToSend.append(254) ; directionsToSend.append(20)
				print(directionsToSend) ; print(weights)
				for num in directionsToSend:
					ser.write(struct.pack('>B',num))
			count += 1
