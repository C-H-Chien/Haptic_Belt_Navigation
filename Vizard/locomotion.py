...
def masterLoop(num):
	...
	'''
	global pathTime, ser, timer, waypoints, waypointCounter, hitWaypoint, sendStopSignal, reorient, constants, waypointPole, motorAngles, controlBlock
	...
	if reorient:
		...
		waypointPole.visible(viz.OFF)#Turn waypoint visibility back on if this part of the control block 
...
def chooseSignal(motorIntensities):
	...
	elif tm.time() - onTimer <= constants.CLICK_ON_TIME:
		dataToSend = [constants.MSG_START] + motorIntensities + [constants.MSG_END]
	...
	#For the rest of the control period, send the belt-off signal 
		...
		if tm.time() - onTimer >= constants.CLICK_ON_PERIOD:
			onTimer = tm.time()
