...
def main_fill_in(subID, experiment_type):
    root = tk.Tk()
    root.title("Vibration Sensation Experiment Questionnaire - Fill in the Blank")

    # Text entry for strategies and observations
    tk.Label(root, text="Did you use any particular strategy in making your responses?").grid(row=0, column=0, padx=10, pady=10)
    strategy_text = tk.Text(root, height=4, width=40)
    strategy_text.grid(row=0, column=1, padx=10, pady=10)

    tk.Label(root, text="Is there anything else you noticed about the experiment?").grid(row=1, column=0, padx=10, pady=10)
